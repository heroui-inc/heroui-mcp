...
async function getLatestVersion() {
  try {
    // Fetch version from GitHub beta branch package.json (same as extractor)
    const url = 'https://raw.githubusercontent.com/heroui-inc/heroui-native/beta/package.json';
...
async function main() {
  ...
  console.log(`HeroUI Native Components: stored=${storedVersion || 'none'}, latest=${latestVersion}, needsUpdate=${results.components.needsUpdate}`);
  ...
  console.log(`HeroUI Native Theme: stored=${storedVersion || 'none'}, latest=${latestVersion}, theme.json exists=${themeFileExists}, needsUpdate=${results.theme.needsUpdate}`);
