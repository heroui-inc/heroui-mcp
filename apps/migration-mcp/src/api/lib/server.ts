...
2. **get_migration_workflow**: Get the user-facing migration workflow guide. Use this for reference if you need the user's perspective on migration steps.

3. **list_component_migration_guides**: List all available component-specific migration guides. Use this to discover which components have migration documentation.

4. **get_component_migration_guides**: Get migration guides for one or more components. Includes component-specific API changes, prop migrations, code examples, and breaking changes. Accepts a components array parameter.
...
2. Invoke \`create-migration-plan\` prompt to create a full migration plan
...
2. Use \`list_component_migration_guides\` to see all available component guides
3. Use \`get_component_migration_guides\` with a components array for specific migration instructions when migrating components
4. Use \`get_styling_migration_guide\` for styling migration after components are migrated
5. Use \`get_migration_workflow\` only if you need the user-facing perspective for reference
...
- Use \`get_component_migration_guides\` when migrating specific components
- Use \`get_styling_migration_guide\` after dependency switch

Example:
- \`get_agent_migration_guide()\` - **Start here** - Get agent guide
- \`list_component_migration_guides()\` - List all components
- \`get_component_migration_guides({ components: ["button"] })\` - Get button migration guide
- \`get_component_migration_guides({ components: ["button", "card", "modal"] })\` - Get multiple component guides
