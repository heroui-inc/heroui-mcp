...
describe("Themes API", () => {
  describe("GET /themes", () => {
    it("should return theme system data", async () => {
      ...
      const data = (await res.json()) as ThemeSystemResponse;
  ...
  describe("GET /themes/variables", () => {
    it("should return theme variables", async () => {
      ...
      const data = (await res.json()) as ThemeVariablesResponse;
      expect(data).toHaveProperty("themes");
    ...
    it("should handle mode parameter", async () => {
      const res = await SELF.fetch("http://localhost:8787/themes/variables?theme=default&mode=light");
  ...
  describe("GET /themes/colors", () => {
    it("should return color variables", async () => {
      ...
      const data = (await res.json()) as ThemeColorsResponse;
      expect(data).toHaveProperty("themes");
      expect(data).toHaveProperty("count");
      expect(Array.isArray(data.themes)).toBe(true);
    });
  ...
  describe("GET /themes/animations", () => {
    it("should return animation definitions", async () => {
      ...
      const data = (await res.json()) as ThemeAnimationsResponse;
  ...
  describe("GET /themes/versions", () => {
    it("should return available theme versions", async () => {
      const res = await SELF.fetch("http://localhost:8787/themes/versions");

      expect(res.status).toBe(200);

      const data = (await res.json()) as ThemeVersionsResponse;
