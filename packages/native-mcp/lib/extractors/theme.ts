...
import type {NativeTheme, NativeThemeSystem} from "../../src/lib/native-types";

import {BaseExtractor} from "./base";
import {ThemeParser} from "./theme-parser";

export type {NativeThemeSystem};
...
export class ThemeExtractor extends BaseExtractor {
  ...
  async extract(): Promise<{data: NativeThemeSystem}> {
   ...
   */
  private async extractDefaultTheme(): Promise<NativeTheme> {
   ...
   */
  private async extractCustomThemes(): Promise<Record<string, NativeTheme>> {
    const allThemes: Record<string, NativeTheme> = {};
